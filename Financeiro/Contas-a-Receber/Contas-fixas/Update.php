...
$campos = ['nome', 'descricao', 'categoria', 'valor', 'recorrencia', 'dia_vencimento', 'data_fim', 'data_vencimento', 'data_pagamento'];
$setters = [];
$params = [];

foreach ($campos as $campo) {
    if (array_key_exists($campo, $data)) {
        if ($campo === 'data_fim' && $data[$campo] === "") {
            $setters[] = "$campo = NULL";
        } else {
            $setters[] = "$campo = :$campo";
...
$sql = "UPDATE financeiro_contas_fixas SET " . implode(", ", $setters) . " WHERE id = :id";
$params[':id'] = $id;
